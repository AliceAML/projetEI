...
def makeEI(nom, point="·"):
    radical = (
        nom[0] + nom[1:].lower()
    )  # je met tout sauf la première lettre en minuscule
    fem_exposant = "e"  # possibilité de le faire à la toute fin ?

    if radical.endswith(
        "e"
    ):  # tentative très brutale de traiter les mots épicènes (ex: violoniste)
        return radical

    elif radical in exceptions:
        fem_exposant = exceptions[radical]
        point = (
            "/"  # si deux formes totalement différentes, on peut utiliser un slash ?
        )
        ...
        fem_exposant = (
            radical[-1] + "e"
        )  # on ajoute la dernière lettre à l'exposant féminin + le e (on pourrait juste ajouter le e tout à la fin aussi)
        ...
        fem_exposant = "que"
    elif radical.endswith(
        "teur"
    ):  # on voit sur ces deux règles (eur/teur) que l'ordre est important
        fem_exposant = "trice"
...
if __name__ == "__main__":
    print(
        """Bienvenue dans ce petit prototype d'assistant d'EI très très basique.\n
    Attention, ce programme fait encore beaucoup d'erreurs !\n"""
    )

    user_input = input(
        "Rentre un nom commun au masculin singulier afin de voir une version inclusive : \n\t"
    )
    printEI(user_input)
